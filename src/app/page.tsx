// page_corrigido_v5.tsx
// Correção: Agrupamento de municípios menos representativos no gráfico de pizza
// e manutenção da renderização condicional para WordCloud
...
const transformDataForChart = (
  dataObject: Record<string, number> | undefined
): ChartData[] => {
  if (!dataObject || Object.keys(dataObject).length === 0) return [];
  return Object.entries(dataObject).map(([name, value]) => ({ name, value }));
};

// Nova função para agrupar municípios no gráfico de Pizza
const transformDataForMunicipioPieChart = (
  dataObject: Record<string, number> | undefined,
  threshold: number = 3 // Municípios com contagem menor que threshold serão agrupados
): ChartData[] => {
  if (!dataObject || Object.keys(dataObject).length === 0) return [];

  const significantData: ChartData[] = [];
  let othersCount = 0;

  Object.entries(dataObject).forEach(([name, value]) => {
    if (value >= threshold) {
      significantData.push({ name, value });
    } else {
      othersCount += value;
    }
  });
  ...
  if (othersCount > 0) {
    significantData.push({ name: "Outros", value: othersCount });
  }
  // Ordenar para que "Outros" apareça por último, se desejado, ou por valor
  significantData.sort((a, b) => b.value - a.value);
...
const HomePage: React.FC = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        setAppData({
          ...
          respostas_interesses: Array.isArray(data.respostas_interesses)
            ? data.respostas_interesses
            : [],
          respostas_mensagem_gestores_transito: Array.isArray(
            data.respostas_mensagem_gestores_transito
          )
            ? data.respostas_mensagem_gestores_transito
            : [],
          respostas_completas_filtragem: Array.isArray(
            data.respostas_completas_filtragem
          )
            ? data.respostas_completas_filtragem
            : [],
        });
      ...
      <DashboardLayout>
        <div className="flex justify-center items-center h-screen">
          <p className="text-2xl text-red-500">
            Erro ao carregar dados: {error || "Dados não puderam ser carregados."}
          </p>
        </div>
      </DashboardLayout>
    );
  ...
  const municipiosUnicos =
    respostasFiltragem.length > 0
      ? [
          "Todos",
          ...new Set(
            respostasFiltragem.map((item) => item?.municipio).filter((m) => m != null)
          ),
        ]
      : ["Todos"];
  ...
  const idadesUnicas =
    respostasFiltragem.length > 0
      ? [
          "Todos",
          ...new Set(respostasFiltragem.map((item) => item?.idade).filter((i) => i != null)),
        ]
      : ["Todos"];

  const filteredData =
    respostasFiltragem.length > 0
      ? respostasFiltragem.filter(
          (item) =>
            item &&
  ...
  // Utiliza a nova função para o gráfico de municípios
  const distribuicaoMunicipioData = transformDataForMunicipioPieChart(
    appData.distribuicao_municipio,
    3 // Agrupa municípios com menos de 3 ocorrências
  );
  const interessesWordCloudData = transformDataForWordCloud(
    appData.respostas_interesses
  );
  const mensagensGestoresWordCloudData = transformDataForWordCloud(
    appData.respostas_mensagem_gestores_transito
  );
  ...
  const filteredInteresseCursoData = recalculateChartData(
    "interesse_curso_transito"
  );
  ...
  return (
    <DashboardLayout>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <p className="text-4xl font-bold">
            {appData?.total_respondentes ?? "N/A"}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow col-span-1 md:col-span-2 lg:col-span-1">
          <h3 className="text-xl font-semibold mb-4 text-blue-600">
            Distribuição por Município (Geral)
          </h3>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
                data={distribuicaoMunicipioData}
                dataKey="value"
                nameKey="name"
                cx="50%"
                cy="50%"
        ...
        <div className="bg-white p-6 rounded-lg shadow col-span-1 md:col-span-2 lg:col-span-1">
          <h3 className="text-xl font-semibold mb-4 text-blue-600">
            Perfil dos Respondentes (Filtrado)
          </h3>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart
              data={filteredDistribuicaoPerfilData}
              layout="vertical"
        ...
        <div className="bg-white p-6 rounded-lg shadow col-span-1 md:col-span-2 lg:col-span-1">
          <h3 className="text-xl font-semibold mb-4 text-blue-600">
            Posse de CNH (Filtrado)
          </h3>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
                data={filteredDistribuicaoCnhData}
                dataKey="value"
                nameKey="name"
                cx="50%"
                cy="50%"
        ...
        <div className="bg-white p-6 rounded-lg shadow col-span-1 md:col-span-2 lg:col-span-1">
          <h3 className="text-xl font-semibold mb-4 text-blue-600">
            Interesse em Curso de Trânsito (Filtrado)
          </h3>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart
              data={filteredInteresseCursoData}
              margin={{ top: 5, right: 20, left: 10, bottom: 5 }}
            >
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis dataKey="name" />
        ...
        {/*mensagensGestoresWordCloudData &&
          mensagensGestoresWordCloudData.length > 0 && (
            <div className="bg-white p-6 rounded-lg shadow col-span-1 md:col-span-3">
              <h3 className="text-xl font-semibold mb-4 text-blue-600">
                Mensagens aos Gestores (Nuvem de Palavras - Geral)
              </h3>
              <ResponsiveContainer width="100%" height={400}>
                <WordCloud
                  words={mensagensGestoresWordCloudData}
                  options={{ ...options, fontSizes: [14, 40] }}
                />
              </ResponsiveContainer>
            </div>
          )*/}
